...
function Fingerprint(inputNode, options) {
  ...
  this.ignoreInAssetMap = options.ignoreInAssetMap;
  this.includeInAssetMap = options.includeInAssetMap;
...
Fingerprint.prototype.writeAssetMap = function (destDir) {
  var ignoreInAssetMap = this.ignoreInAssetMap;
  var includeInAssetMap = this.includeInAssetMap;
  var assets = this.assetMap;

  for (var i = 0; i < ignoreInAssetMap.length; i++) {
    for (var key in assets) {
      if (key.indexOf(ignoreInAssetMap[i]) > -1) {
        delete assets[key];
      }
    }
  }

  for (var i = 0; i < includeInAssetMap.length; i++) {
    for (var key in assets) {
      if (key.indexOf(includeInAssetMap[i]) === -1) {
        delete assets[key];
      }
    }
  }
